...
Status: released to production. The production build endpoint reported
`version: 0.2.0`, `channel: production`, `environment: production`, and
`branch: main` on 2026-07-16. The deployed commit `d5da1cd` is tagged
[`v0.2.0`](https://github.com/shchilkin/PopChoice/releases/tag/v0.2.0). Tracked by
...
Production verification captured on 2026-07-16:

```json
{
  "version": "0.2.0",
  "channel": "production",
  "commitShortSha": "d5da1cd",
  "branch": "main",
  "environment": "production"
}
```

The public health endpoint also reported PostgreSQL and Redis as healthy. See
the live [`/api/build`](https://pop-choice.shchilkin.dev/api/build) and
[`/api/health`](https://pop-choice.shchilkin.dev/api/health) endpoints. These
endpoints are current runtime evidence; the snapshot above records what was
observed on the stated date.
...
- The documented production promotion path is: merge `development`
...
- Production was verified after promotion with:
