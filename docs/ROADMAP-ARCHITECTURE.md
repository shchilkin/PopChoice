...
- Migrate the dedicated backoffice app to Next.js in [#596](https://github.com/shchilkin/PopChoice/issues/596) before adding larger interactive operator workflows. Keep the `apps/backoffice` deployment boundary, but use React/Next so catalog health, TMDB reviews, pagination, bulk repair flows, and shared PopChoice UI conventions do not have to be rebuilt in hand-written HTML/CSS/vanilla JS.
- Add pagination or virtualization for large backoffice operator tables in [#591](https://github.com/shchilkin/PopChoice/issues/591), covering catalog-health issue rows and TMDB review queues without rendering full high-cardinality datasets at once.
- Add safe bulk catalog repair enqueueing in [#593](https://github.com/shchilkin/PopChoice/issues/593), with count previews, bounded batches, deduped `catalog-maintenance` jobs, audit summaries, and worker-side TMDB/OpenAI pacing preserved.
