...
| `REDIS_URL`                | Web, workers, BullMQ, Bull Board, rate limiting | Optional local; required for queued recommendation flow in prod; e2e maps from `E2E_REDIS_URL`                         | Queue features disable or use limited inline fallback when missing  | Secret if it contains credentials. Future backoffice screens only need it for queue state. |
...
| `PORT`                     | Web, Bull Board, backoffice                     | Optional                                                                                                               | Web/backoffice `3000`; Bull Board `4000` or script-specific default | In Coolify the public proxy usually routes to container port `3000`.                       |
...
| `OPERATOR_AUTH_USERNAME`   | Bull Board, future backoffice                   | Optional local; required for protected production operator surfaces                                                    | None                                                                | Basic Auth username for operational/admin UIs.                                             |
| `OPERATOR_AUTH_PASSWORD`   | Bull Board, future backoffice                   | Optional local; required for protected production operator surfaces                                                    | None                                                                | Secret. Set with `OPERATOR_AUTH_USERNAME`; rotate like an admin password.                  |
| `OPERATOR_AUTH_REALM`      | Bull Board, future backoffice                   | Optional                                                                                                               | `PopChoice Operators`                                               | Browser login prompt realm.                                                                |
| `OPERATOR_AUTH_REQUIRED`   | Bull Board, future backoffice                   | Optional                                                                                                               | `true` in production, `false` locally                               | Set `0` only for intentionally private/local operator surfaces.                            |
...
| `TMDB_LANGUAGE`                     | Discovery/backfill/enqueue scripts | Optional          | `en-US`                                                                         | TMDB locale tag such as `fi-FI` or `ru-RU`.                        |
...
| `CATALOG_HEALTH_FORMAT`             | Catalog health report              | Optional          | `text`                                                                          | Use `json` for machine-readable output.                            |
| `CATALOG_HEALTH_SAMPLE_LIMIT`       | Catalog health report              | Optional          | `5`                                                                             | Sample rows/groups per issue.                                      |
| `CATALOG_HEALTH_STALE_DAYS`         | Catalog health report              | Optional          | `180`                                                                           | Stale TMDB metadata threshold.                                     |
