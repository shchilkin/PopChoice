...
| `DATABASE_URL`             | Web, workers, services, migrations              | Required for app DB behavior; CI/e2e may use `E2E_DATABASE_URL` instead                                                | Local setup generates it                                           | Secret if it contains credentials. PostgreSQL must support pgvector.                       |
| `REDIS_URL`                | Web, workers, BullMQ, Bull Board, rate limiting | Optional local; required for queued recommendation flow in prod; e2e maps from `E2E_REDIS_URL`                         | Queue features disable or use limited inline fallback when missing | Secret if it contains credentials.                                                         |
...
| `PORT`                     | Web, Bull Board                                 | Optional                                                                                                               | Web `3000`; Bull Board `4000` or script-specific default           | In Coolify the public proxy usually routes to container port `3000`.                       |
