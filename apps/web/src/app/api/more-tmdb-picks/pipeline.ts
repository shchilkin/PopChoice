    ...
    const batchResults = await Promise.all(
      batch.map(async (tmdb) => {
        ...
        try {
          ...
          if (detailRes.ok) {
            const detail = tmdbMovieDetailSchema.safeParse(await detailRes.json());
            if (detail.success) {
              if (detail.data.title) localizedTitle = detail.data.title;
              if (detail.data.runtime) runtime = detail.data.runtime;
              if (detail.data.overview) localizedOverview = detail.data.overview;
            }
          }
        } catch {
        ...
        try {
          ...
          logger.warn(
            { err, movieTitle: tmdb.title, isRateLimit },
            'more-picks pipeline: AI description failed, using fallback',
          );
          if (locale !== 'en' && localizedOverview) {
            try {
              const translation = await getOpenAIClient().chat.completions.create({
                model: MODELS.MINI,
                messages: [
                  {
                    role: 'system',
              ...
              aiDescription = translation.choices[0]?.message?.content?.trim() ?? localizedOverview;
            } catch {
        ...
        return {
          id: -tmdb.id, // negative TMDB ID convention
          name: localizedTitle,
          year,
